...
struct ImageProcessingConstants {
    ...
    static let optimalDimensionX : Int = 28
    static let optimalDimensionY : Int = 28
    // For Edge Detection
...
class ImagePreProcessing {
    ...
    public func preProcessImage(image : UIImage ) -> (UIImage, [Double]) {
        ...
        let cropimage = cropImage(image: image)
        
        // Resize the image
        let smallSizeImage = resizeImage(image: cropimage, targetSize: CGSize(width: 2.0 * 28.0, height: 2.0 * 28.0))
    ...
    fileprivate func normalizedIntensityMatrix(image: UIImage) -> [Double] {
        ...
        // pixel calculation  , logMatrix is 28*28
        let logMatrix = logPixelOfImage(image: image)
        printMatrix(inputMatrix: logMatrix, name : "Pixels")
        ...
        return create1DMatrix(inputMatrix: suppressedMatrix)
    }
